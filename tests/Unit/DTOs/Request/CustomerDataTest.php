...
it('creates customer data with minimal fields', function (): void {
    ...
    expect($customer->accountId)->toBeNull();
...
it('creates customer data with all fields', function (): void {
    ...
    expect($customer->accountId)->toBe('acc_123');
    expect($customer->email)->toBe('test@example.com');
    expect($customer->fullName)->toBe('John Doe');
    expect($customer->phone)->toBe('+1234567890');
    expect($customer->ip)->toBe('192.168.1.1');
    expect($customer->fingerprint)->toBe('fp_123');
    expect($customer->address)->toBe('123 Main St');
    expect($customer->city)->toBe('New York');
    expect($customer->state)->toBe('NY');
    expect($customer->postalCode)->toBe('10001');
    expect($customer->country)->toBe('US');
    expect($customer->neighborhood)->toBe('Manhattan');
    expect($customer->birthdate)->toBeInstanceOf(DateTimeImmutable::class);
});
...
it('serializes to JSON', function (): void {
    ...
    expect($json)->toHaveKeys(['email', 'fullName', 'ip']);
...
it('does not include null fields in JSON', function (): void {
    ...
    expect($json)->not->toHaveKey('accountId');
