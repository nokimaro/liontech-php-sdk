...
it('creates refund request with minimal data', function (): void {
    ...
    expect($request->amount->amount)->toBe('50.00');
    expect($request->paymentId)->toBe('pay_123');
    expect($request->webhookUrl)->toBeNull();
    expect($request->customFields)->toBeNull();
});
...
it('creates refund request with all data', function (): void {
    $request = new CreateRefundRequest(
        ...
        customFields: ['refund_reason' => 'customer_request'],
    );

    expect($request->webhookUrl)->toBe('https://example.com/webhook');
    expect($request->customFields)->toBe(['refund_reason' => 'customer_request']);
});
...
it('serializes to JSON', function (): void {
    ...
    expect($json)->toHaveKeys(['amount', 'paymentId']);
    expect($json['amount'])->toBe(['value' => '50.00', 'currency' => 'USD']);
...
it('serializes optional fields when present', function (): void {
    ...
    expect($json)->toHaveKeys(['webhookUrl', 'customFields']);
...
it('does not include null fields in JSON', function (): void {
    ...
    expect($json)->not->toHaveKey('webhookUrl');
    expect($json)->not->toHaveKey('customFields');
});
