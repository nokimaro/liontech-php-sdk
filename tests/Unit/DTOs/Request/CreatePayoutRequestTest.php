...
it('creates payout request with all data', function (): void {
    ...
    $request = new CreatePayoutRequest(
        ...
        customFields: ['payout_ref' => 'ref_123'],
    );
    ...
    expect($request->customFields)->toBe(['payout_ref' => 'ref_123']);
});
...
it('serializes to JSON', function (): void {
    ...
    expect($json)->toHaveKeys(['amount', 'paymentData']);
...
it('serializes optional fields when present', function (): void {
    ...
    expect($json)->toHaveKeys(['customer', 'orderId', 'webhookUrl', 'customFields']);
