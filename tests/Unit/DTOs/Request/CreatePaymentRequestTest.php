...
it('creates payment request with all data', function (): void {
    ...
    expect($request->autoApprove)
        ->toBeFalse();
    expect($request->backLink)
        ->toBe('https://example.com/back');
...
it('serializes to JSON', function (): void {
    ...
    expect($json)
        ->toHaveKeys(['amount', 'paymentData', 'autoApprove', 'description']);
...
it('serializes optional fields when present', function (): void {
    ...
    expect($json)
        ->toHaveKeys(['customer', 'orderId', 'backLink', 'webhookUrl', 'customFields', 'options']);
