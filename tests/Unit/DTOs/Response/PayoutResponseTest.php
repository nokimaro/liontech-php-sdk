...
it('creates payout response from array', function (): void {
    ...
    expect($response->payoutId)->toBe('payout_123');
    expect($response->amount)->toBeInstanceOf(Money::class);
    expect($response->amount->amount)->toBe('500.00');
    expect($response->amount->currency)->toBe(Currency::USD);
    expect($response->status)->toBe(PayoutStatus::PENDING);
});
...
it('handles optional fields', function (): void {
    $data = [
        ...
        'orderId' => 'ord_123',
        'amount' => ['value' => '500.00', 'currency' => 'USD'],
        'convAmount' => ['value' => '450.00', 'currency' => 'EUR'],
        'status' => 'SUCCEEDED',
        'createdAt' => '2024-01-01T00:00:00Z',
    ...
    expect($response->convAmount->amount)->toBe('450.00');
    expect($response->paymentMethod)->toBe(['type' => 'card']);
    expect($response->webhookUrl)->toBe('https://example.com/webhook');
    expect($response->txnId)->toBe('txn_123');
    expect($response->rrn)->toBe('rrn_123');
});
...
it('handles missing optional fields', function (): void {
    ...
    expect($response->convAmount)->toBeNull();
    expect($response->paymentMethod)->toBeNull();
    expect($response->customFields)->toBeNull();
    expect($response->txnId)->toBeNull();
...
it('handles missing createdAt', function (): void {
    ...
    expect($response->createdAt)->toBeInstanceOf(DateTimeImmutable::class);
});
